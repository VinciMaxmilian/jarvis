# Graph Report - jarvis  (2026-08-04)

## Corpus Check
- 314 files · ~369,329 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4272 nodes · 9152 edges · 239 communities (187 shown, 52 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 481 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0891949a`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- CapabilityRegistry
- gemini_provider.py
- ollama_provider.py
- test_shell.py
- filesystem/backend/handlers.py
- test_contracts.py
- SistemaDeArquivos
- TestClient
- FakeLLMProvider
- test_architecture.py
- MCPClientManager
- MemorySystem
- goals.py
- test_cf_access.py
- InMemoryGoalStore
- What You Must Do When Invoked
- Jarvis — Sistema Operacional Cognitivo Pessoal
- http/schemas/models.py
- CapabilityPermissions
- FakeJwksEndpoint
- test_model_profiles.py
- chief.py
- compilerOptions
- Settings
- jobs.py
- 1. Stack por camada
- routers/settings.py
- InMemoryConversationStore
- tauri.conf.json
- What You Must Do When Invoked
- compilerOptions
- dependencies
- devDependencies
- Jarvis — Plano de execução v1 → v3
- test_scheduler_jobs.py
- CloudflareAccessMiddleware
- 1. O que já está PRONTO e FUNCIONANDO (Fase v1 Completa)
- contracts.py
- JwksCache
- get_settings
- Runbook da infraestrutura
- test_agent_profiles.py
- package.json
- manifest.json
- NeuralEngine
- ToolSpec
- llm/base.py
- graphify reference: extra exports and benchmark
- test_memory_working.py
- env.py
- Mapa do Repositório e Política de Documentação
- FastAPI
- .claude/CLAUDE.md
- graphify reference: query, path, explain
- README.md
- caveman.md
- React + TypeScript + Vite
- graphify reference: add a URL and watch a folder
- graphify reference: commit hook and native CLAUDE.md integration
- graphify reference: incremental update and cluster-only
- packages/agents
- Capability SDK
- tsconfig.json
- graphify reference: GitHub clone and cross-repo merge
- graphify reference: transcribe video and audio
- FakeEmbeddingProvider
- Capability
- Problema
- CapabilityHarness
- CLAUDE.md
- extraction-spec.md
- packages/__init__.py
- tests/__init__.py
- unit/__init__.py
- jarvis
- Handoff — trabalho interrompido no meio (2026-07-30)
- permissions
- test_capability_sdk_manifest.py
- graphify reference: extra exports and benchmark
- plugins
- src/App.tsx
- ._recusar
- react
- test_http.py
- graphify reference: query, path, explain
- graphify reference: add a URL and watch a folder
- graphify reference: commit hook and native CLAUDE.md integration
- graphify reference: incremental update and cluster-only
- graphify reference: GitHub clone and cross-repo merge
- graphify reference: transcribe video and audio
- AGENTS.md
- .agents/skills/graphify/references/extraction-spec.md
- dependencies
- PythonRuntime
- memory/__init__.py
- ._carregar
- PermissionPolicy
- ManifestLoadError
- capability_id
- InMemoryKnowledgeIndex
- test_backup.py
- exemplo_nas/backend/handlers.py
- ExecutionRequest
- ExperienceRecord
- CleanupService
- GoalStore
- LLMProvider
- AsyncSubprocessRunner
- ChatScreen.tsx
- InProcEventBus
- expo
- test_memory_knowledge.py
- SchedulerConfig
- SettingsScreen.tsx
- useChatStore.ts
- kernel/errors.py
- mobile/App.tsx
- _mock_provider
- CapabilityManifest
- registry/__init__.py
- FakeCapabilityStore
- HistoryScreen.tsx
- SystemToolExecutor
- .from_database_url
- graph.ts
- _child.py
- desk/package.json
- ProfiledToolExecutor
- GoalsScreen.tsx
- BackupService
- InMemoryVectorStore
- Módulo Mobile – Integração com RunAnywhere SDK
- O que precisa de validação manual
- 20260731T235635Z/manifest.json
- 20260801T000118Z/manifest.json
- memory_writer/backend/handlers.py
- Handoff — 31/07/2026 (madrugada de 01/08)
- Resposta
- deps.py
- git/backend/handlers.py
- compute_capability_digest
- 2. Interface (UI) e Estabilidade Corrigidas
- KnowledgeDocument
- .execute_next_task
- ChatPage.tsx
- exemplo_nas — capability de exemplo do SDK
- PromptNotFound
- guard_tools
- mobile/tsconfig.json
- restore.sh
- Plano de Implementação: Conversação por Voz (Jarvis)
- filesystem — arquivos e pastas dentro do que o dono concedeu
- default.json
- cap_loop.py
- cap_ok.py
- cap_rede.py
- CapabilityHandler
- shell — executar um programa externo, com coleira
- ToolPolicy
- remark-math
- tailwind-merge
- Event
- http — buscar e enviar por HTTP, só para hosts concedidos
- backend/__init__.py
- backup.sh
- RuntimeAdapter
- lib.rs
- Jarvis Desktop (Tauri v2)
- .get_state
- test_health_e_estado_medido_e_nao_status
- MarkdownRenderer.tsx
- discover_and_register_tools
- MCP Skills
- shell/backend/__init__.py
- @picovoice/porcupine-react
- @picovoice/web-voice-processor
- react
- ._recusar
- rehype-katex
- @tauri-apps/plugin-autostart
- @tauri-apps/plugin-dialog
- filesystem/backend/__init__.py
- .flush
- test_adaptador_postgres_implementa_a_porta
- apolo_desktop_agent
- mcp_discord
- memoria
- Http
- Planos — Design Industry, Voz Live, Self-Evolution, Capabilities
- history.py
- test_url_invalida_morre_no_schema
- PermissaoNaoDeclarada
- ExecutionStatus
- .mark_used
- _FakeSession
- caminho_seguro
- .embed
- parametrize
- test_prompt_vem_do_arquivo_e_nao_do_codigo
- ._integridade_ok
- CapabilityRecord
- Memória: knowledge
- Memória: chat_history
- Memória: chat_history
- Memória: chat_history
- Memória: chat_history
- Memória: chat_history
- Memória: chat_history
- Memória: chat_history
- Memória: chat_history
- Memória: knowledge
- .list_indexed
- .upsert
- construir
- http/backend/__init__.py
- rag/__init__.py
- remark-gfm

## God Nodes (most connected - your core abstractions)
1. `ToolSpec` - 103 edges
2. `CapabilityPermissions` - 102 edges
3. `CapabilityRegistry` - 74 edges
4. `Event` - 70 edges
5. `InMemoryGoalStore` - 62 edges
6. `Problema` - 60 edges
7. `tool()` - 58 edges
8. `Goal` - 57 edges
9. `Task` - 55 edges
10. `CapabilityHarness` - 52 edges

## Surprising Connections (you probably didn't know these)
- `FakeJwksEndpoint` --uses--> `AccessTokenError`  [INFERRED]
  tests/unit/test_cf_access.py → apps/api/cf_access.py
- `FakeJwksEndpoint` --uses--> `JwksCache`  [INFERRED]
  tests/unit/test_cf_access.py → apps/api/cf_access.py
- `FakeJwksEndpoint` --uses--> `CloudflareAccessVerifier`  [INFERRED]
  tests/unit/test_cf_access.py → apps/api/cf_access.py
- `FakeJwksEndpoint` --uses--> `CloudflareAccessMiddleware`  [INFERRED]
  tests/unit/test_cf_access.py → apps/api/cf_access.py
- `SystemSettingsRow` --uses--> `CapabilityHealth`  [INFERRED]
  apps/api/db/models.py → packages/registry/records.py

## Import Cycles
- None detected.

## Communities (239 total, 52 thin omitted)

### Community 0 - "CapabilityRegistry"
Cohesion: 0.08
Nodes (59): CapabilityRegistry, Catálogo em memória, com estado durável atrás da porta `CapabilityStore`., escrever_capability(), Path, `discover()`, `get_active()`, `resolve()` e a persistência do Capability…, Nome do diretório é a chave em disco; divergência é capability ambígua., Id derivado do nome: sem isso cada boot criaria uma linha nova na tabela., D-2: o canal do miss é o evento, não a exceção. Inverso do teste que fixava o… (+51 more)

### Community 1 - "gemini_provider.py"
Cohesion: 0.08
Nodes (40): _as_dict(), _as_int(), _as_list(), _as_str(), _attach_images(), _bare_model(), _conforms_to_protocol(), _error_detail() (+32 more)

### Community 2 - "ollama_provider.py"
Cohesion: 0.08
Nodes (34): ToolCall, _as_dict(), _as_int(), _as_str(), _attach_images(), _conforms_to_protocol(), _error_detail(), _looks_like_embedding_unsupported() (+26 more)

### Community 3 - "test_shell.py"
Cohesion: 0.04
Nodes (82): carregar_allowlist(), construir(), _cortar(), Executor, executor_subprocess(), BaseModel, Capability, NoReturn (+74 more)

### Community 4 - "filesystem/backend/handlers.py"
Cohesion: 0.24
Nodes (21): O que a capability `filesystem` faz: ler, escrever e reorganizar arquivos. É a…, Schemas de entrada e saída da capability `filesystem`., ApagarEntrada, ApagarSaida, CopiarEntrada, CopiarSaida, CriarPastaEntrada, CriarPastaSaida (+13 more)

### Community 5 - "test_contracts.py"
Cohesion: 0.08
Nodes (29): TaskStatus, _manifest(), GoalStatus, parametrize, Invariantes dos contratos de `packages/shared/contracts.py`. Os contratos são a…, `can_retry` fora de FAILED reagendaria task em andamento., Timestamp naive em base com timezone é bug silencioso de 3 horas., `plan.md` §6: o miss do registry publica exatamente este tipo. (+21 more)

### Community 6 - "SistemaDeArquivos"
Cohesion: 0.08
Nodes (39): Capability, Lê, escreve e reorganiza arquivos dentro das pastas concedidas., SistemaDeArquivos, capability(), concessao(), fixture, Path, Teste da capability `filesystem`. `plan-scheme.md`: uma capability sem `tests/`… (+31 more)

### Community 7 - "TestClient"
Cohesion: 0.17
Nodes (28): RSAPrivateKey, TestClient, _hdr(), make_token(), O Access também deposita o token no cookie `CF_Authorization`. Ele é fallback,…, A mesma conta Zero Trust assina token para todas as suas aplicações. Sem…, Token assinado, no prazo e para esta aplicação — e ainda assim recusado.…, `kid` conhecido, formato perfeito, chave errada — o caso que só a verificação… (+20 more)

### Community 8 - "FakeLLMProvider"
Cohesion: 0.06
Nodes (65): GoalManager, Gerencia ciclo de vida de Goals e Tasks., FakeLLMProvider, ChatMessage, Enfileira uma `Completion` montada à mão (tokens, finish_reason)., Quantas respostas roteirizadas ainda não foram consumidas., `ToolExecutor` que registra chamadas em vez de executar. O Chief AI e o…, `LLMProvider` roteirizado. Sem rede, sem aleatoriedade. `complete()` consome a… (+57 more)

### Community 9 - "test_architecture.py"
Cohesion: 0.13
Nodes (29): arquivos_python(), coletar_imports(), ImportRef, nomes_importados(), _pacote_do_arquivo(), _proibido_para_o_chief(), parametrize, Path (+21 more)

### Community 10 - "MCPClientManager"
Cohesion: 0.12
Nodes (12): MCPClientManager, MCPServerInstance, Any, Path, Gerenciador de Clientes MCP (Model Context Protocol). Descobre servidores MCP…, Devolve as especificações das ferramentas prontas para o LLM., Invoca uma ferramenta roteando para o servidor MCP correto., Encerra todas as conexões e recarrega os servidores MCP. (+4 more)

### Community 11 - "MemorySystem"
Cohesion: 0.12
Nodes (13): Working memory for active tasks and goals using Redis., Deletes stored state., ShortTermMemory, _alvo(), MemorySystem, Goal, Task, Bloco de contexto para o planejamento (`GoalManager.decompose_goal`). É aqui… (+5 more)

### Community 12 - "goals.py"
Cohesion: 0.21
Nodes (19): create_goal(), execute_goal(), get_goal(), list_goals(), list_tasks(), AsyncSession, get, Goal (+11 more)

### Community 13 - "test_cf_access.py"
Cohesion: 0.10
Nodes (31): extract_token(), Token do header do Access; na falta dele, do cookie `CF_Authorization`., parametrize, Gate do Cloudflare Access: verificação do JWT na origem. A suíte assina os…, O HEALTHCHECK do Dockerfile bate aqui de dentro do container e não tem como…, Rota que ninguém registrou à mão também é coberta — é a razão de o gate ser…, `BaseHTTPMiddleware` só é chamado para scope `http`; o canal principal do…, O default não pode quebrar `127.0.0.1:8000`, onde não existe Access e portanto… (+23 more)

### Community 14 - "InMemoryGoalStore"
Cohesion: 0.08
Nodes (42): CreateGoalRequest, BaseModel, UpdateGoalStatusRequest, FileAccessRule, Goal, BaseModel, Unidade de trabalho do sistema. Sobrevive ao fechamento da janela., Passo executável de um Goal. Executado por uma capability, nunca pelo Chief AI. (+34 more)

### Community 15 - "What You Must Do When Invoked"
Cohesion: 0.07
Nodes (26): For /graphify add and --watch, For /graphify query, For the commit hook and native CLAUDE.md integration, For --update and --cluster-only, /graphify, Honesty Rules, Interpreter guard for subcommands, Part A - Structural extraction for code files (+18 more)

### Community 16 - "Jarvis — Sistema Operacional Cognitivo Pessoal"
Cohesion: 0.07
Nodes (26): 10. Memória, 11. Eventos, scheduler e agentes contínuos, 12. Modelos, 13. Cliente PWA, 14. Roadmap, 15. Decisões resolvidas, 16. Fora de escopo, 1. Objetivo (+18 more)

### Community 17 - "http/schemas/models.py"
Cohesion: 0.11
Nodes (20): O host da URL contra `permissions.network`. É a conferência de escopo.…, O caminho comum das duas tools: confere escopo, chama, embrulha., Schemas de entrada e saída da capability `http`., _Comum, GetEntrada, _headers_seguros(), host_de(), PostEntrada (+12 more)

### Community 18 - "CapabilityPermissions"
Cohesion: 0.04
Nodes (73): F, abrir_navegador(), executar_comando_cmd(), Abre o navegador padrão na máquina local., Executa um comando genérico no terminal/CMD do sistema local., abrir_discord(), Abre o aplicativo do Discord no computador do Senhor., Salva uma preferência ou anotação importante do usuário na memória de longo… (+65 more)

### Community 19 - "FakeJwksEndpoint"
Cohesion: 0.12
Nodes (18): MockTransport, FakeJwksEndpoint, _jwk(), jwks(), Any, `/cdn-cgi/access/certs` em memória, com contador de acessos. O contador é o que…, Cache frio com 20 requisições simultâneas: o lock tem de deixar UMA passar. Sem…, Rotação: a chave girou e o token chega com `kid` que nunca vimos. (+10 more)

### Community 20 - "test_model_profiles.py"
Cohesion: 0.06
Nodes (58): build_llm_provider_for_profile(), profile_override(), Collection, Override do dono para (provider, perfil), ou "" se não declarado. `getattr`…, Perfil → modelo para o provider dado. Não vai à rede e não levanta. `served` é…, Provider já apontado para o modelo do perfil. Nunca levanta por perfil. É a…, resolve_profile_model(), _log() (+50 more)

### Community 21 - "chief.py"
Cohesion: 0.14
Nodes (17): UUID, Chief AI — núcleo mínimo da v0. Recebe mensagem do usuário, envia para LLM com…, agent_provider(), AgentProfile, get_agent_profile(), load_prompt(), Perfil de agente → prompt, tools, temperatura e modelo. Antes disto o `ChiefAI`…, Um papel: prompt, tools, temperatura e para onde vai a chamada de LLM. (+9 more)

### Community 22 - "compilerOptions"
Cohesion: 0.08
Nodes (23): compilerOptions, allowArbitraryExtensions, allowImportingTsExtensions, erasableSyntaxOnly, jsx, lib, module, moduleDetection (+15 more)

### Community 23 - "Settings"
Cohesion: 0.06
Nodes (26): _build_anthropic(), _build_gemini(), _build_lmstudio(), _build_ollama(), _build_openai_compatible(), LM Studio na LAN. Mesma classe do `openai` — o LM Studio serve a API OpenAI —…, Serve tanto `openai` quanto `local` — LM Studio, vLLM e Koboldcpp falam a mesma…, Entrada separada de `local` de propósito: o Ollama é servido pela API nativa… (+18 more)

### Community 24 - "jobs.py"
Cohesion: 0.05
Nodes (49): Backup: `pg_dump` do Postgres + snapshot do diretório do LanceDB. O risco R-5…, Lê e valida o `manifest.json` de um backup. Usado por ferramenta e teste., Confere os digests do manifesto contra o disco. Devolve a lista de problemas —…, read_manifest(), verify_backup(), Limpeza: arquivos de log velhos e chaves de short-term memory que vazaram. Duas…, Scheduler package — os três jobs recorrentes da v1.4. Nada aqui importa…, Scheduler: os três jobs recorrentes da v1.4 (`plan-execution.md` §3, D-11).… (+41 more)

### Community 25 - "1. Stack por camada"
Cohesion: 0.09
Nodes (22): 1.1 Frontend web (PWA), 1.2 Backend, 1.3 LLM e inferência, 1.4 Dados, 1.5 RAG e parsing de documentos, 1.6 Busca web, controle do computador, mídia, 1.7 Observabilidade, 1.8 Containers e qualidade (+14 more)

### Community 26 - "routers/settings.py"
Cohesion: 0.07
Nodes (50): SystemSettingsRow, build_llm_provider(), provider_default_model(), Providers atendíveis agora, em ordem de preferência (não alfabética). Fonte…, Modelo default de um provider, ou "" se o provider não existe., Ids que o provider diz servir agora, ou `None` se não deu para saber. `None`…, Resolve o provider pelo mapa. Desconhecido falha nomeando os válidos. Fora do…, served_models() (+42 more)

### Community 27 - "InMemoryConversationStore"
Cohesion: 0.18
Nodes (21): ChiefAI, Chief AI v0: LLM loop com tool calling, sob um perfil de agente., O papel em vigor. Exposto para log e diagnóstico., InMemoryConversationStore, `ConversationStore` em lista, na ordem de escrita., _executor_com_catalogo(), LLMQueRegistraTemperatura, `FakeLLMProvider` que guarda a temperatura de cada chamada. (+13 more)

### Community 28 - "tauri.conf.json"
Cohesion: 0.09
Nodes (22): app, security, windows, build, beforeBuildCommand, beforeDevCommand, devUrl, frontendDist (+14 more)

### Community 29 - "What You Must Do When Invoked"
Cohesion: 0.07
Nodes (26): For /graphify add and --watch, For /graphify query, For the commit hook and native AGENTS.md integration, For --update and --cluster-only, /graphify, Honesty Rules, Interpreter guard for subcommands, Part A - Structural extraction for code files (+18 more)

### Community 30 - "compilerOptions"
Cohesion: 0.10
Nodes (19): compilerOptions, allowImportingTsExtensions, erasableSyntaxOnly, lib, module, moduleDetection, noEmit, noFallthroughCasesInSwitch (+11 more)

### Community 31 - "dependencies"
Cohesion: 0.11
Nodes (19): dependencies, clsx, @picovoice/porcupine-web, react-dom, react-markdown, react-router, @tanstack/react-query, @tauri-apps/api (+11 more)

### Community 32 - "devDependencies"
Cohesion: 0.09
Nodes (23): devDependencies, autoprefixer, oxlint, postcss, tailwindcss, @types/node, @types/react, @types/react-dom (+15 more)

### Community 33 - "Jarvis — Plano de execução v1 → v3"
Cohesion: 0.09
Nodes (21): 1. Estado real hoje (auditado, não declarado), 2. Triagem das sugestões estruturais, 2b. Triagem da segunda rodada de sugestões (2026-07-30), 3. v1 — Sistema utilizável de verdade, 4. v2 — Capabilities escritas à mão, 5. v3 — Self-evolution, 6. Ordem e dependências, 7. Invariantes (+13 more)

### Community 34 - "test_scheduler_jobs.py"
Cohesion: 0.07
Nodes (37): BackupError, Backup não terminou. O evento `backup.completed` **não** é emitido., Starts the APScheduler and registers jobs., Backup do Postgres (`pg_dump`) e do diretório do LanceDB. Emite…, Log antigo e chaves de short-term memory sem TTL., Reindexação incremental do knowledge., Publica no bus. Bus ausente ou com defeito não invalida o job feito., Registra e roda os jobs periódicos do sistema. (+29 more)

### Community 35 - "CloudflareAccessMiddleware"
Cohesion: 0.33
Nodes (6): CloudflareAccessMiddleware, Gate ASGI puro: nega por default, libera `/health` por exceção. **Por que…, ASGIApp, Receive, Scope, Send

### Community 36 - "1. O que já está PRONTO e FUNCIONANDO (Fase v1 Completa)"
Cohesion: 0.09
Nodes (21): 1. Capabilities (Prioridade Máxima), 1. O que já está PRONTO e FUNCIONANDO (Fase v1 Completa), 2. Event Bus — concluído, 2. O que está EM ANDAMENTO ou PENDENTE (O Futuro — v2 e v3), 3. Débitos Técnicos Residuais a Resolver, 3. Papéis Especializados, 4. Planos Possíveis e Diretrizes de Foco, 4. Self Evolution e Autoconsciência (+13 more)

### Community 37 - "contracts.py"
Cohesion: 0.06
Nodes (56): Base, CapabilityRow, ChatMessageRow, ConversationRow, GoalRow, SQLAlchemy ORM tables derivados dos contratos Pydantic. Regra: contracts.py é a…, Catálogo de capabilities (v1.1). Espelha…, Base declarativa. Alembic importa daqui. (+48 more)

### Community 38 - "JwksCache"
Cohesion: 0.11
Nodes (22): AccessTokenError, CloudflareAccessVerifier, _http_transport(), install_cloudflare_access(), JwksCache, Any, Exception, FastAPI (+14 more)

### Community 39 - "get_settings"
Cohesion: 0.10
Nodes (28): dispose_engine(), get_engine(), get_session_factory(), AsyncSession, Async SQLAlchemy engine e session factory. Única fonte de `AsyncSession` no…, _configure_logging(), create_app(), lifespan() (+20 more)

### Community 40 - "Runbook da infraestrutura"
Cohesion: 0.05
Nodes (37): 1. Pré-requisito único: o `.env`, 2. Subir tudo, 3. Migrations (Alembic), 4. Inferência: fora do Docker, de propósito, 5. Portas expostas, 6. Derrubar, 7. Quando algo não sobe, 8.1 A decisão: conector como serviço do Windows, no host (+29 more)

### Community 41 - "test_agent_profiles.py"
Cohesion: 0.07
Nodes (30): Modelo para um papel, com a razão da escolha. Nunca levanta. Fina de propósito:…, resolve_agent_model(), Perfis de agente: prompt de arquivo, tools por papel, modelo e temperatura. O…, Dois papéis com o mesmo texto seriam o mesmo papel com dois nomes., Regressão da própria refatoração. O texto saiu de uma constante em `chief.py`…, A contrapartida: negar o desconhecido ao executor o deixaria sem nada para…, O papel histórico. Apertar a política dele mudaria o comportamento da v0., Ordem, não valores: execução não é lugar para variedade, e planejar admite mais… (+22 more)

### Community 42 - "package.json"
Cohesion: 0.20
Nodes (9): name, private, scripts, build, dev, lint, preview, type (+1 more)

### Community 43 - "manifest.json"
Cohesion: 0.20
Nodes (9): background_color, description, display, icons, name, orientation, short_name, start_url (+1 more)

### Community 44 - "NeuralEngine"
Cohesion: 0.06
Nodes (20): decodeA(), decodeW(), encodeA(), encodeW(), GraphLink, GraphNode, graphPromises, hex2rgb() (+12 more)

### Community 45 - "ToolSpec"
Cohesion: 0.10
Nodes (22): Processa mensagem do usuário. Yield StreamChunks para streaming., Chama o LLM com a temperatura do perfil. Os dois ramos existem porque…, AnthropicProvider, Any, Provider Anthropic — implementação concreta de LLMProvider. Usado pelo Chief AI…, Converte ToolSpec → formato Anthropic tool_use., Separa system prompt e converte Messages → formato Anthropic., LLMProvider concreta para API Anthropic. Suporta complete, stream e NÃO embed. (+14 more)

### Community 46 - "llm/base.py"
Cohesion: 0.10
Nodes (23): ChatCompletionMessageToolCall, ContentBlocked, LLMError, ProviderRequestError, Exception, Camada de abstração de LLM. Escrita ANTES do primeiro provider. Nenhum módulo…, Base de toda falha da camada de LLM., O provider não implementa a operação. Quem chama decide o fallback. (+15 more)

### Community 47 - "graphify reference: extra exports and benchmark"
Cohesion: 0.22
Nodes (8): graphify reference: extra exports and benchmark, Step 6b - Wiki (only if --wiki flag), Step 7 - Neo4j export (only if --neo4j or --neo4j-push flag), Step 7a - FalkorDB export (only if --falkordb or --falkordb-push flag), Step 7b - SVG export (only if --svg flag), Step 7c - GraphML export (only if --graphml flag), Step 7d - MCP server (only if --mcp flag), Step 8 - Token reduction benchmark (only if total_words > 5000)

### Community 48 - "test_memory_working.py"
Cohesion: 0.10
Nodes (38): InMemoryWorkingMemoryStore, JsonFileWorkingMemoryStore, `WorkingMemoryStore` em dicionário. Não sobrevive ao processo, de propósito., `WorkingMemoryStore` em disco: um arquivo por task. Um arquivo por task, e não…, Descarta o estado. A vida útil do nível é a da task (`plan.md` §10)., Estado vivo da task, com persistência a cada checkpoint., WorkingMemory, _matar_no_meio_da_task() (+30 more)

### Community 49 - "env.py"
Cohesion: 0.36
Nodes (7): get_sync_url(), Alembic env.py — usa os ORM models de apps.api.db.models. O `target_metadata`…, Esconde a senha de um DSN antes de ele ir para log. O DSN carrega credencial.…, Lê a URL do settings (async) e converte para sync., _redact(), run_migrations_offline(), run_migrations_online()

### Community 50 - "Mapa do Repositório e Política de Documentação"
Cohesion: 0.25
Nodes (7): Documentação sob demanda, Documentos permitidos, Estrutura do monorepo, Git, Layout de uma capability em disco, Mapa do Repositório e Política de Documentação, Sincronização casa ↔ trabalho

### Community 51 - "FastAPI"
Cohesion: 0.13
Nodes (19): app(), app_real(), client(), _generate_key(), key_alheia(), keypair(), FastAPI, fixture (+11 more)

### Community 52 - ".claude/CLAUDE.md"
Cohesion: 0.33
Nodes (5): Auto-Clarity, graphify, Intensity, Persistence, Rules

### Community 53 - "graphify reference: query, path, explain"
Cohesion: 0.33
Nodes (5): For /graphify explain, For /graphify path, graphify reference: query, path, explain, Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal

### Community 55 - "caveman.md"
Cohesion: 0.40
Nodes (4): Auto-Clarity, Intensity, Persistence, Rules

### Community 56 - "React + TypeScript + Vite"
Cohesion: 0.50
Nodes (3): Expanding the Oxlint configuration, React Compiler, React + TypeScript + Vite

### Community 57 - "graphify reference: add a URL and watch a folder"
Cohesion: 0.50
Nodes (3): For /graphify add, For --watch, graphify reference: add a URL and watch a folder

### Community 58 - "graphify reference: commit hook and native CLAUDE.md integration"
Cohesion: 0.50
Nodes (3): For git commit hook, For native CLAUDE.md integration, graphify reference: commit hook and native CLAUDE.md integration

### Community 59 - "graphify reference: incremental update and cluster-only"
Cohesion: 0.50
Nodes (3): For --cluster-only, For --update (incremental re-extraction), graphify reference: incremental update and cluster-only

### Community 60 - "packages/agents"
Cohesion: 0.40
Nodes (4): A restrição de tools é allowlist, Arquivos, packages/agents, Perfis de agente

### Community 61 - "Capability SDK"
Cohesion: 0.33
Nodes (5): Capability SDK, Escrever uma capability, Gerar o manifest, Superfície, Testar

### Community 67 - "FakeEmbeddingProvider"
Cohesion: 0.13
Nodes (20): FakeEmbeddingProvider, `LLMProvider` cuja única razão de existir é `embed()`. Esta máquina não tem…, Total de textos vetorizados desde o início. Zero é uma asserção válida., make_base(), ACEITE v1.3 (2): ingerir um documento novo e achá-lo por uma pergunta. A…, Duas perguntas, dois assuntos, dois documentos — o índice discrimina., Incremental" é isto e nada mais: conteúdo igual, trabalho nenhum., Reformatar um arquivo não pode custar uma rodada inteira de embeddings. (+12 more)

### Community 68 - "Capability"
Cohesion: 0.07
Nodes (38): Capability, _coletar(), concedido(), _conferir_assinatura(), _conferir_identidade(), _dentro(), especificacoes(), _Marcador (+30 more)

### Community 69 - "Problema"
Cohesion: 0.05
Nodes (47): NoReturn, NoReturn, NoReturn, NoReturn, alvos_nao_concedidos(), Ensaio, O que a tool faria, sem ter feito. Retorno de `call(dry_run=True)`. `executado`…, Os pares `(kind, target)` que a tool exige e o manifest não concede. (+39 more)

### Community 70 - "CapabilityHarness"
Cohesion: 0.09
Nodes (53): CapabilityHarness, Capability, Path, Verifica e exercita uma capability instalada em disco. A instância recebida é a…, Onde a capability poderia escrever: o diretório dela mais a concessão., capability(), concessao(), FalarEntrada (+45 more)

### Community 81 - "Handoff — trabalho interrompido no meio (2026-07-30)"
Cohesion: 0.12
Nodes (15): 0. Estado da árvore, 1. Como rodar qualquer coisa, 2. Placar agora, 3. Agente A — Cloudflare Tunnel (infraestrutura), 4. Agente B — validação do JWT do Access na origem, 5. Ordem sugerida para retomar, 6. O que o dono precisa fazer no dashboard, Entregue e no disco (+7 more)

### Community 83 - "permissions"
Cohesion: 0.10
Nodes (20): description, identifier, main, permissions, platforms, $schema, windows, autostart:default (+12 more)

### Community 84 - "test_capability_sdk_manifest.py"
Cohesion: 0.13
Nodes (46): carregar_arquivos(), manifest_de(), Capability, Lê e valida `manifest.yaml` + `permissions.yaml` de uma capability. Raises:…, Monta o `CapabilityManifest` a partir da declaração em código. Só três coisas…, campos(), escrever(), manifest_base() (+38 more)

### Community 85 - "graphify reference: extra exports and benchmark"
Cohesion: 0.22
Nodes (8): graphify reference: extra exports and benchmark, Step 6b - Wiki (only if --wiki flag), Step 7 - Neo4j export (only if --neo4j or --neo4j-push flag), Step 7a - FalkorDB export (only if --falkordb or --falkordb-push flag), Step 7b - SVG export (only if --svg flag), Step 7c - GraphML export (only if --graphml flag), Step 7d - MCP server (only if --mcp flag), Step 8 - Token reduction benchmark (only if total_words > 5000)

### Community 86 - "plugins"
Cohesion: 0.22
Nodes (8): plugins, rules, react/only-export-components, react/rules-of-hooks, $schema, oxc, typescript, warn

### Community 87 - "src/App.tsx"
Cohesion: 0.14
Nodes (10): BrainPage, HistoryPage, MemoryPage, RulesPage, ToolsPage, LayoutProps, MOBILE_TABS, PageId (+2 more)

### Community 88 - "._recusar"
Cohesion: 0.12
Nodes (14): _contar_arquivos(), EscreverEntrada, EscreverSaida, ListarEntrada, ListarSaida, NoReturn, Path, A primeira pasta concedida. É contra ela que o relativo é resolvido. Levantar… (+6 more)

### Community 89 - "react"
Cohesion: 0.09
Nodes (27): VoiceButton(), getApiBase(), Theme, ThemeContext, ThemeContextType, ThemeProvider(), useTheme(), useDesktopIntegration() (+19 more)

### Community 90 - "test_http.py"
Cohesion: 0.12
Nodes (23): capability(), cliente(), ClienteEspiao, concessao(), Exception, fixture, Teste da capability `http`. `plan-scheme.md`: uma capability sem `tests/` não…, A negação vem antes da chamada — não é a resposta que é descartada. (+15 more)

### Community 91 - "graphify reference: query, path, explain"
Cohesion: 0.33
Nodes (5): For /graphify explain, For /graphify path, graphify reference: query, path, explain, Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal

### Community 92 - "graphify reference: add a URL and watch a folder"
Cohesion: 0.50
Nodes (3): For /graphify add, For --watch, graphify reference: add a URL and watch a folder

### Community 93 - "graphify reference: commit hook and native CLAUDE.md integration"
Cohesion: 0.50
Nodes (3): For git commit hook, For native CLAUDE.md integration, graphify reference: commit hook and native CLAUDE.md integration

### Community 94 - "graphify reference: incremental update and cluster-only"
Cohesion: 0.50
Nodes (3): For --cluster-only, For --update (incremental re-extraction), graphify reference: incremental update and cluster-only

### Community 99 - "dependencies"
Cohesion: 0.04
Nodes (48): dependencies, axios, expo, expo-constants, expo-crypto, @expo/ngrok, expo-secure-store, expo-status-bar (+40 more)

### Community 100 - "PythonRuntime"
Cohesion: 0.06
Nodes (41): ExecutionLimits, ExecutionResult, BaseModel, O que volta ao Executive. Nunca levanta: falha é campo, não exceção.…, Decodifica e corta, dizendo que cortou. Silenciosamente cortar a saída é como…, Limites do isolamento. Docker por capability é v2 (`plan-execution.md`). Os…, `cpu_seconds` explícito, ou o teto de parede + 1 s de folga., truncar() (+33 more)

### Community 101 - "memory/__init__.py"
Cohesion: 0.04
Nodes (80): Nível `experience` — padrões extraídos de execução. `plan.md` §10: "é o nível…, Formata os padrões para o prompt de planejamento. O bloco diz o que fazer com a…, render_experience_context(), build_memory_system(), build_vector_store(), Path, Montagem dos níveis a partir de configuração. O backend de vetores é escolhido…, Os cinco níveis prontos para injeção, com persistência em disco. `short_term`… (+72 more)

### Community 103 - "._carregar"
Cohesion: 0.09
Nodes (12): _escrever_atomico(), InMemoryKnowledgeIndex, JsonFileExperienceStore, JsonFileKnowledgeIndex, _ler_json(), IndexedDocument, Path, UUID (+4 more)

### Community 104 - "PermissionPolicy"
Cohesion: 0.08
Nodes (29): _envolver_caminhos(), _host_de(), _instalar_filesystem(), _instalar_processo(), _instalar_rede(), install(), _liberado(), O wrapper que faz a permissão negar de verdade, dentro do subprocesso. `plan-… (+21 more)

### Community 105 - "ManifestLoadError"
Cohesion: 0.15
Nodes (16): CapabilityGapDetected, InvalidCapabilityStateError, ManifestLoadError, Exception, Erros do Capability Registry., Lacuna de capacidade. **Não é mais levantada por `resolve()`** (D-2): desde a…, `manifest.yaml` ausente, ilegível ou fora do schema., Capability não está no estado exigido pela operação. (+8 more)

### Community 106 - "capability_id"
Cohesion: 0.14
Nodes (9): capability_id(), Capability, UUID, Entrada mínima e `disabled` para uma capability que não carregou. O nome do…, Todas as capabilities carregadas, inclusive as `disabled`., Só as que podem executar (`status == active`)., Casa uma intenção contra o catálogo `active`. `None` é o miss. Miss **não…, Id estável derivado do nome — a chave da capability em disco. `uuid4()` a cada… (+1 more)

### Community 107 - "InMemoryKnowledgeIndex"
Cohesion: 0.11
Nodes (32): InMemoryKnowledgeIndex, KnowledgeIndex, IndexedDocument, Catálogo do que já foi indexado: `doc_id` → hash + ids dos chunks. É o que…, InMemoryKnowledgeIndex, datetime, IndexedDocument, KnowledgeDocument (+24 more)

### Community 108 - "test_backup.py"
Cohesion: 0.15
Nodes (35): FakePgDump, make_service(), make_target(), datetime, Path, `BackupService` — o que o job escreve no disco e quando ele se recusa. R-5…, O digest é o contrato com o restore: se mentir, o restore recusa backup bom., Um byte trocado tem de reprovar — senão o digest é decoração. (+27 more)

### Community 109 - "exemplo_nas/backend/handlers.py"
Cohesion: 0.06
Nodes (45): construir(), NasArquivos, Capability, ListarEntrada, ListarSaida, Protocol, O que a capability `exemplo_nas` faz. Molde das capabilities reais da v2. Esta…, A capability sob a concessão que está no manifest em disco. É a fábrica que o… (+37 more)

### Community 110 - "ExecutionRequest"
Cohesion: 0.14
Nodes (28): ExecutionRequest, Uma chamada de tool prestes a acontecer., BusDeTeste, kernel_com_bus(), manifesto(), Path, Os três aceites da v1.2, exercidos contra subprocesso de verdade. `plan-…, A contraprova do teste de negação: o que foi declarado tem de passar. Um guarda… (+20 more)

### Community 111 - "ExperienceRecord"
Cohesion: 0.10
Nodes (15): ExperienceMemory, UUID, Como o dono costuma decidir — o terceiro conteúdo do nível (`plan.md` §10)., Registros que já passaram do limiar, do mais recorrente ao menos. É a **única**…, Acumula e promove padrões de execução., Registra (ou incrementa) a falha de uma capability/tool., Registra que a capability funcionou. Contrapeso das falhas., error_signature() (+7 more)

### Community 112 - "CleanupService"
Cohesion: 0.11
Nodes (23): CleanupService, datetime, Path, Poda arquivos de log por idade e recolhe chaves efêmeras sem TTL., envelhecer(), FakeKeyspace, Path, `CleanupService` — poda de log por idade e recolhimento de chave sem TTL. Os… (+15 more)

### Community 113 - "GoalStore"
Cohesion: 0.05
Nodes (28): main(), Orchestrator v0.5 — Executive Function + GoalManager. Instancia o loop de…, Boot: cria deps, resume goals, entra no loop., Executive, Loop de controle sobre goals ativos., Boot: resume interrupted goals, then enter poll loop., Enfileira goal para processamento., Main loop: processa goals da fila + poll por novos. (+20 more)

### Community 114 - "LLMProvider"
Cohesion: 0.05
Nodes (43): generate_graph_html(), get_graphify_html(), get_memory_graph_json(), get_memory_html(), get, post, Rota de memória — serve a visualização de grafo dos vetores RAG., Retorna o HTML com o grafo do VectorStore num JSON para driblar o Cloudflare. (+35 more)

### Community 115 - "AsyncSubprocessRunner"
Cohesion: 0.13
Nodes (26): AsyncSubprocessRunner, `CommandRunner` sobre `asyncio`. Herda o ambiente e acrescenta o que veio.…, MonkeyPatch, python_c(), `AsyncSubprocessRunner` — a única peça do scheduler que fala com o SO. Todo o…, Sem isto, um `pg_dump` pendurado segura o job até o backup do dia seguinte., `errors="replace"`: saída suja de um binário não pode virar exceção., `PGPASSWORD` é passado por `env`, nunca por `argv` (ver `PostgresTarget`). (+18 more)

### Community 116 - "ChatScreen.tsx"
Cohesion: 0.18
Nodes (16): styles, InputBar(), InputBarProps, styles, MessageBubble, styles, styles, Props (+8 more)

### Community 117 - "InProcEventBus"
Cohesion: 0.06
Nodes (18): InProcEventBus, EventHandler, Eventos aceitos e ainda não entregues., Entrega tudo que já está na fila e devolve quantos foram entregues.…, Sobe o laço de despacho em segundo plano. Idempotente., Drena o que sobrou e encerra o laço. Idempotente., Um consumidor e os tipos de evento que ele quer. `types=None` é "tudo" — usado…, Nome legível do consumidor, para o log de falha dizer quem quebrou. (+10 more)

### Community 118 - "expo"
Cohesion: 0.07
Nodes (26): backgroundColor, backgroundImage, foregroundImage, monochromeImage, adaptiveIcon, package, predictiveBackGestureEnabled, expo (+18 more)

### Community 119 - "test_memory_knowledge.py"
Cohesion: 0.14
Nodes (17): chunk_id(), chunk_text(), Divide o texto em pedaços de até `size` caracteres, com sobreposição. A…, Id estável do chunk. Reingerir o mesmo documento reescreve as mesmas linhas., parametrize, Nível `knowledge` — aceite 2 da v1.3 (`plan-execution.md` §3). > "Documento…, Ids estáveis dependem disto: chunk diferente a cada passada duplicaria tudo., Texto sem espaço não pode entrar em laço no recuo até o último espaço. (+9 more)

### Community 120 - "SchedulerConfig"
Cohesion: 0.17
Nodes (12): BaseSettings, Path, Configuração dos jobs, lida do ambiente com prefixo `JARVIS_`. Por que **não**…, Caminho absoluto; relativo é resolvido contra a raiz do repositório., Onde cada job escreve, quanto guarda e a que horas roda., resolve_path(), SchedulerConfig, main() (+4 more)

### Community 121 - "SettingsScreen.tsx"
Cohesion: 0.25
Nodes (13): getSystemSettings(), listProfiles(), listProviders(), ProfileAssignment, ProfileCatalog, ProviderCatalog, ProviderOption, SystemSettings (+5 more)

### Community 122 - "useChatStore.ts"
Cohesion: 0.10
Nodes (15): ChatChunk, ChatHttpResponse, ChatSocket, ChatSocketHandlers, RECONNECT_DELAYS_MS, RNWebSocket, sendChatOverHttp(), ToolCallPayload (+7 more)

### Community 123 - "kernel/errors.py"
Cohesion: 0.11
Nodes (18): ExecutionFailed, ExecutionTimeout, KernelError, PermissionDenied, Exception, Erros do kernel de execução. Todos derivam de `KernelError` para que quem chama…, Base dos erros do kernel., `manifest.runtime` não tem adapter registrado. Erro de configuração, não de… (+10 more)

### Community 124 - "mobile/App.tsx"
Cohesion: 0.11
Nodes (22): App(), navigationTheme, Stack, styles, Tab, TAB_GLYPH, probeSession(), LOGIN_URL (+14 more)

### Community 125 - "_mock_provider"
Cohesion: 0.13
Nodes (19): _mock_provider(), Request, Response, Provider com transporte falso. Devolve também as requisições vistas: o que…, Resposta com a MESMA forma medida no LM Studio, inclusive `index`., O bug que isto trava: `embed()` indo para o modelo de conversa. No LM Studio os…, Alguns backends respondem 400 a `input: []`; um erro fantasma por lista vazia é…, A especificação permite devolver fora de ordem; vetor casado com o texto errado… (+11 more)

### Community 126 - "CapabilityManifest"
Cohesion: 0.09
Nodes (30): _conferir_chaves(), _conferir_entrypoint(), _conferir_permissions_yaml(), escrever_arquivos(), _intents(), _ler_mapa(), _problemas_de_validacao(), Any (+22 more)

### Community 127 - "registry/__init__.py"
Cohesion: 0.16
Nodes (18): Capability Registry — catálogo, casamento de intenção e integridade. Superfície…, _avisar_ambiguidade(), build_index(), casa(), CatalogIndex, frase(), match_intent(), normalizar() (+10 more)

### Community 128 - "FakeCapabilityStore"
Cohesion: 0.24
Nodes (6): FakeCapabilityStore, Any, O motivo de a tabela existir: dicionário em memória morre no `restart`., `CapabilityStore` em dicionário, com a semântica do `PgCapabilityStore`. Guarda…, test_estado_operacional_sobrevive_ao_restart(), test_fake_store_implementa_a_porta()

### Community 129 - "HistoryScreen.tsx"
Cohesion: 0.21
Nodes (13): ChatPreview, getStats(), listChatMessages(), listChats(), ModelStats, StatsResponse, StoredChatMessage, ToolUsage (+5 more)

### Community 130 - "SystemToolExecutor"
Cohesion: 0.23
Nodes (5): _as_int(), ToolExecutor com as tools do sistema., Devolve as specs do sistema e dos MCPs., Síncrono (legado). Apenas system tools., SystemToolExecutor

### Community 131 - ".from_database_url"
Cohesion: 0.13
Nodes (19): _lancedb_dir(), KnowledgeIndex, Path, Diretório de vetores, lido do ambiente **sem importar `lancedb`**.…, Monta os três serviços a partir do DSN da aplicação e do ambiente. Recebe o DSN…, Aceita `postgresql://`, `postgresql+asyncpg://` e `postgresql+psycopg://`., parametrize, `PostgresTarget` — a tradução do DSN da aplicação para argumentos do libpq.… (+11 more)

### Community 132 - "graph.ts"
Cohesion: 0.19
Nodes (11): BrainGraph, BrainLink, BrainNode, downsample(), fetchBrainGraph(), RawLink, RawNode, BrainCanvas() (+3 more)

### Community 133 - "_child.py"
Cohesion: 0.21
Nodes (15): IO, negados(), Alvos negados até agora, na ordem: `["fs:/etc/passwd", "net:1.1.1.1:80"]`., _aceita_dry_run(), _ensaio_minimo(), _envelope(), main(), Any (+7 more)

### Community 134 - "desk/package.json"
Cohesion: 0.15
Nodes (12): description, devDependencies, @tauri-apps/cli, name, private, scripts, build, dev (+4 more)

### Community 135 - "ProfiledToolExecutor"
Cohesion: 0.24
Nodes (5): ProfiledToolExecutor, `ToolExecutor` visto através da política de um perfil. Satisfaz a mesma porta…, O executor envolvido. Exposto para diagnóstico, não para desvio., Catálogo completo (sistema + MCP), filtrado. Delega ao `get_all_specs` de baixo…, `False` para tool negada, mesmo que o executor de baixo a tenha. Mentir aqui é…

### Community 136 - "GoalsScreen.tsx"
Cohesion: 0.16
Nodes (16): api, describeError(), SessionProbe, createGoal(), executeGoal(), Goal, GoalStatus, listGoals() (+8 more)

### Community 137 - "BackupService"
Cohesion: 0.21
Nodes (8): BackupService, datetime, Path, Executa o backup completo. Levanta `BackupError` se algo falhar., Cópia do diretório de vetores. Ausência é caso normal, não erro. Nunca importa…, Carimbo UTC; sufixo `-N` se dois backups caírem no mesmo segundo., Backups **íntegros**, do mais antigo ao mais novo. Íntegro = tem…, Produz um diretório de backup autoconferível. Layout resultante::…

### Community 138 - "InMemoryVectorStore"
Cohesion: 0.13
Nodes (10): InMemoryVectorStore, Path, Salva os vetores no arquivo JSON (de forma síncrona). Usamos I/O síncrono…, `VectorStore` em dicionário, com cosseno em Python puro. É o adapter…, Carrega os vetores do arquivo JSON de forma síncrona (boot)., Uma loja, dois níveis: o namespace é o que os mantém separados., `plan.md` §10: o nível `long` é **editável**, e id estável é o que permite., test_fato_editado_substitui_em_vez_de_acumular() (+2 more)

### Community 139 - "Módulo Mobile – Integração com RunAnywhere SDK"
Cohesion: 0.12
Nodes (15): Arquitetura, Filosofia da Arquitetura, Gerenciamento de Recursos, Inferência Local, Modelos, Módulo Mobile – Integração com RunAnywhere SDK, Objetivo, Objetivos da integração (+7 more)

### Community 140 - "O que precisa de validação manual"
Cohesion: 0.15
Nodes (12): 1. Login no Cloudflare Access (o item de maior risco), 2. Chat, 3. Brain, 4. Metas, 5. Ajustes, Autenticação — os dois modos, Fora de escopo, registrado, Jarvis Mobile (+4 more)

### Community 141 - "20260731T235635Z/manifest.json"
Cohesion: 0.15
Nodes (12): backup_id, created_at, database, lancedb, manifest_version, postgres, bytes, files (+4 more)

### Community 142 - "20260801T000118Z/manifest.json"
Cohesion: 0.15
Nodes (12): backup_id, created_at, database, lancedb, manifest_version, postgres, bytes, files (+4 more)

### Community 143 - "memory_writer/backend/handlers.py"
Cohesion: 0.22
Nodes (9): construir(), MemoryWriter, Capability, EscreverEntrada, EscreverSaida, Guarda anotações na memória de longo prazo., EscreverEntrada, EscreverSaida (+1 more)

### Community 144 - "Handoff — 31/07/2026 (madrugada de 01/08)"
Cohesion: 0.20
Nodes (9): Ambiente, Bugs achados por execução (não por leitura), EM ABERTO — bolhas de chat vazias (não resolvido), Estado medido, não declarado, Handoff — 31/07/2026 (madrugada de 01/08), Pendências do dono (fora do código), Restrição desta máquina, Também em aberto (+1 more)

### Community 145 - "Resposta"
Cohesion: 0.17
Nodes (9): Cliente, cliente_httpx(), BaseModel, Protocol, `cliente` é injeção, no mesmo molde da sonda de `exemplo_nas`., O que um cliente devolve. Fronteira entre a biblioteca HTTP e a tool., Faz a requisição. Injetável para o teste não tocar a rede., O cliente real. Fora do teste por construção: a suíte roda sem rede. O corpo é… (+1 more)

### Community 146 - "deps.py"
Cohesion: 0.09
Nodes (36): effective_provider_and_model(), get_capability_registry(), get_chat_history_store(), get_chief_ai(), get_conversation_store(), get_db(), get_goal_store(), get_llm_provider() (+28 more)

### Community 147 - "git/backend/handlers.py"
Cohesion: 0.23
Nodes (10): construir(), _cortar(), Git, Capability, Path, Executa comandos git de forma segura., _texto(), GitEntrada (+2 more)

### Community 148 - "compute_capability_digest"
Cohesion: 0.25
Nodes (10): arquivos_do_digest(), compute_capability_digest(), integridade_confere(), Path, Digest do código em disco de uma capability (D-3). `plan.md` §6: o registry…, Arquivos que compõem o digest, em ordem estável. A ordem é a do caminho…, SHA-256 do conteúdo do diretório da capability, exceto o `manifest.yaml`. O…, `True` quando o código em disco é exatamente o aprovado. (+2 more)

### Community 149 - "2. Interface (UI) e Estabilidade Corrigidas"
Cohesion: 0.25
Nodes (7): 1. Conexão do Agente e Cloudflare Access Resolvidos, 2. Interface (UI) e Estabilidade Corrigidas, A. Comunicação Síncrona do Chat, B. Histórico (HistoryPage), C. Neural Map (BrainPage) Otimizado, Relatório da Sessão: Jarvis & Infraestrutura, Status Atual

### Community 150 - "KnowledgeDocument"
Cohesion: 0.17
Nodes (11): KnowledgeDocument, model_validator, Documento a ingerir. `doc_id` default é a própria fonte — legível no log., Chunking existe para isto: a resposta é o trecho, não o arquivo inteiro., Sem remoção, a base guardaria duas versões e a busca devolveria a antiga., Ingerir vazio deixaria o índice mentindo sobre a cobertura., Id legível no log; `doc_id` explícito continua ganhando., test_conteudo_alterado_apaga_os_chunks_velhos() (+3 more)

### Community 151 - ".execute_next_task"
Cohesion: 0.24
Nodes (7): Goal, Task, UUID, Executa próxima task pendente. Retorna task executada ou None., Processa goal completo: executa tasks até acabar ou falhar., Resume após restart: busca goals ACTIVE e retoma processamento., Usa LLM para decompor goal em tasks.

### Community 152 - "ChatPage.tsx"
Cohesion: 0.24
Nodes (8): Markdown(), MarkdownRenderer, aplicarTexto(), ChatMessage, ChatPage(), NeuralMap, useChat(), WsChunk

### Community 153 - "exemplo_nas — capability de exemplo do SDK"
Cohesion: 0.33
Nodes (5): Credenciais e ambiente, Estado, exemplo_nas — capability de exemplo do SDK, O que copiar daqui, O que faz

### Community 154 - "PromptNotFound"
Cohesion: 0.18
Nodes (8): FileNotFoundError, LookupError, PromptNotFound, Collection, Path, Papel pedido que não existe. Nomeia os válidos porque a causa é sempre erro de…, Arquivo de prompt ausente. Levanta de propósito, ao contrário da resolução de…, UnknownAgentProfile

### Community 155 - "guard_tools"
Cohesion: 0.10
Nodes (20): guard_tools(), Envelopa `tools` na política de `profile`. Envelopa sempre, inclusive quando a…, make_tool_spec(), `ToolSpec` mínima e válida (o `input_schema` é JSON Schema de verdade)., Se não satisfizesse, quem consome teria de saber que está restrito — e a…, O teste central da fatia. Não basta o perfil DIZER que o planner não age: a…, O envelope decide QUEM pode, não COMO executa: `dry_run` é do executor de baixo…, `SystemToolExecutor` tem `get_all_specs` (sistema + MCP) e o dublê não. O… (+12 more)

### Community 156 - "mobile/tsconfig.json"
Cohesion: 0.40
Nodes (4): compilerOptions, strict, extends, expo/tsconfig.base

### Community 157 - "restore.sh"
Cohesion: 0.60
Nodes (3): die(), info(), restore.sh script

### Community 158 - "Plano de Implementação: Conversação por Voz (Jarvis)"
Cohesion: 0.17
Nodes (11): 1. Nova Visão Geral da Arquitetura (Áudio-para-Áudio Nativo), 2. Escolha de Tecnologias e Modelos, 3. Desafio Técnico Principal: Live API e Conexão Contínua, 4. Passos para Implementação (Foco na Live API), 5. Vantagens Absolutas do Ecossistema Atualizado, Fase 1: Prototipação Modular (Fallback), Fase 2: Integração com a Live API (Áudio-para-Áudio), Fase 3: Refinamento de UX e Interrupções (+3 more)

### Community 159 - "filesystem — arquivos e pastas dentro do que o dono concedeu"
Cohesion: 0.20
Nodes (9): Caminho relativo e caminho absoluto, Credenciais e ambiente, Estado, filesystem — arquivos e pastas dentro do que o dono concedeu, Fronteira: três camadas, três papéis, Manutenção, Nenhum `OSError` sobe cru, Riscos e tetos (+1 more)

### Community 160 - "default.json"
Cohesion: 0.22
Nodes (8): description, identifier, main, permissions, $schema, windows, core:default, opener:default

### Community 161 - "cap_loop.py"
Cohesion: 0.50
Nodes (3): handler(), Any, Capability que nunca termina. Existe para provar que o supervisor a mata. Laço…

### Community 162 - "cap_ok.py"
Cohesion: 0.50
Nodes (3): handler(), Any, Capability de exemplo que funciona. Roda de verdade, em subprocesso. Mora sob…

### Community 163 - "cap_rede.py"
Cohesion: 0.50
Nodes (3): handler(), Any, Capability que tenta sair para a rede. Existe para provar que ela não sai. Usa…

### Community 164 - "CapabilityHandler"
Cohesion: 0.25
Nodes (5): CapabilityHandler, Any, Path, Handler para a capability de anotações de preferências., Entrypoint de execução isolado pelo Kernel.

### Community 166 - "shell — executar um programa externo, com coleira"
Cohesion: 0.20
Nodes (9): A allowlist, As quatro proteções, Credenciais e ambiente, Estado, `exit_code != 0` não é falha da tool, Manutenção, Risco, shell — executar um programa externo, com coleira (+1 more)

### Community 167 - "ToolPolicy"
Cohesion: 0.20
Nodes (9): BaseModel, Quais tools um papel pode chamar. Três campos e uma ordem de precedência fixa:…, `True` se este papel pode chamar `tool`., ToolPolicy, Precedência fixa: negar é a decisão mais forte. Sem isso, a ordem de avaliação…, Um `ToolPolicy()` vazio não pode liberar nada: perfil novo escrito às pressas…, test_default_da_politica_e_negar(), test_deny_vence_allow() (+1 more)

### Community 170 - "Event"
Cohesion: 0.06
Nodes (41): Barramento de eventos em processo, sobre `asyncio.Queue`. Primeiro uso real da…, Barramento de eventos usando Redis Streams. Substitui o InProcEventBus para…, Publica de código síncrono numa porta assíncrona. `resolve()` é síncrono porque…, Event, Fato ocorrido. Na v0 trafega por asyncio.Queue; na v2 por Redis Streams., EventBus, asyncio.Queue na v0; Redis Streams na v2. O contrato não muda., conversation_store() (+33 more)

### Community 171 - "http — buscar e enviar por HTTP, só para hosts concedidos"
Cohesion: 0.25
Nodes (7): Concessão, Dependência, Estado, http — buscar e enviar por HTTP, só para hosts concedidos, Por que a conferência é em runtime, Tools, Três decisões que não são detalhe

### Community 174 - "RuntimeAdapter"
Cohesion: 0.33
Nodes (4): Protocol, Quem sabe executar um `runtime`. Um adapter por forma de chamar código.…, Nome canônico que este adapter atende (`python`, `mcp`, `http`)., RuntimeAdapter

### Community 175 - "lib.rs"
Cohesion: 0.60
Nodes (4): AppHandle, reveal_main_window(), run(), started_from_autostart()

### Community 176 - "Jarvis Desktop (Tauri v2)"
Cohesion: 0.40
Nodes (4): Behaviour, Commands, Jarvis Desktop (Tauri v2), Permissions

### Community 177 - ".get_state"
Cohesion: 0.40
Nodes (3): Any, Stores arbitrary state with a TTL., Retrieves stored state.

### Community 178 - "test_health_e_estado_medido_e_nao_status"
Cohesion: 0.67
Nodes (3): parametrize, `status` é decisão humana, `health` é consequência medida — não se misturam., test_health_e_estado_medido_e_nao_status()

### Community 181 - "MCP Skills"
Cohesion: 0.50
Nodes (3): Como funciona, Criando uma habilidade manual, MCP Skills

### Community 209 - "Http"
Cohesion: 0.12
Nodes (15): Http, Capability, Busca e envia por HTTP, só para os hosts concedidos no manifest., Os hosts concedidos, normalizados. Vazio = a capability não sai daqui., O aceite: manifest, catálogo, permissões, layout, dry_run e os dois casos., A conferência é em runtime porque o host vem no argumento, não no código., Injeção de cabeçalho: `\\r\\n` num valor transforma uma requisição em duas., O catálogo é o que o kernel lê antes de repetir ou de perguntar. (+7 more)

### Community 210 - "Planos — Design Industry, Voz Live, Self-Evolution, Capabilities"
Cohesion: 0.11
Nodes (17): A.0 O que a fonte realmente é, A.1 O obstáculo real, A.2 Estratégia: ponte de tokens, depois migração por página, A.3 Riscos, B.1 Topologia, B.2 Etapas, B.3 A confirmar antes de codar, C.1 Máquina de estados (+9 more)

### Community 211 - "history.py"
Cohesion: 0.22
Nodes (16): ChatMessageResponse, ChatPreview, get_chat_messages(), get_chats(), get_stats(), ModelStats, AsyncSession, BaseModel (+8 more)

### Community 212 - "test_url_invalida_morre_no_schema"
Cohesion: 0.67
Nodes (3): parametrize, Esquema errado, host ausente e credencial embutida morrem antes do I/O., test_url_invalida_morre_no_schema()

### Community 213 - "PermissaoNaoDeclarada"
Cohesion: 0.06
Nodes (43): Browser, construir(), Capability, NoReturn, Extrai texto visível de páginas web., ExtrairEntrada, ExtrairSaida, BaseModel (+35 more)

### Community 214 - "ExecutionStatus"
Cohesion: 0.67
Nodes (3): ExecutionStatus, StrEnum, Desfecho de uma execução. Separado do `TaskStatus` de propósito: a task pode…

### Community 216 - "_FakeSession"
Cohesion: 0.18
Nodes (9): _async(), _FakeResult, _FakeSession, Any, MonkeyPatch, Só o suficiente para `effective_provider_and_model`. Sem Postgres: o assunto do…, LM Studio desligado não pode virar 500: o dono consulta esta tela justamente…, test_endpoint_de_perfis_mostra_quem_serve_o_que() (+1 more)

### Community 217 - "caminho_seguro"
Cohesion: 0.31
Nodes (4): caminho_seguro(), field_validator, Recusa o caminho que sai da raiz concedida, antes de qualquer I/O. Args: valor:…, ValidationInfo

### Community 219 - "parametrize"
Cohesion: 0.20
Nodes (10): parametrize, `criar_servidor_mcp` escreve arquivo no repo e recarrega o processo. É a única…, Buscar não tem efeito colateral: negar isso ao planner só o faria planejar…, O caso que uma denylist erraria: um servidor MCP novo aparece em runtime com um…, Filtrar o catálogo é ergonomia — o modelo não pede o que não vê. A garantia é o…, test_catalogo_visto_pelo_papel_restrito_nao_contem_a_tool_de_acao(), test_leitura_e_liberada_para_todo_papel(), test_papel_que_nao_age_nao_pode_criar_servidor_mcp() (+2 more)

### Community 220 - "test_prompt_vem_do_arquivo_e_nao_do_codigo"
Cohesion: 0.33
Nodes (7): clear_prompt_cache(), Esvazia o cache. Para o teste que troca arquivo de prompt entre casos., Path, A propriedade que dá sentido a `prompts/*.md`: trocar o arquivo troca o prompt,…, Ao contrário da resolução de modelo, aqui não há degradação sensata: um agente…, test_prompt_ausente_levanta_em_vez_de_degradar(), test_prompt_vem_do_arquivo_e_nao_do_codigo()

### Community 221 - "._integridade_ok"
Cohesion: 0.25
Nodes (4): Path, D-3: o código em disco tem de ser o que passou pelo gate., Raiz do catálogo em disco. Público desde a v1.2: o kernel precisa do diretório…, Diretório em disco de uma capability. Não confere existência.

### Community 222 - "CapabilityRecord"
Cohesion: 0.11
Nodes (11): CapabilityStore, datetime, Protocol, Persistência do catálogo. `runtime_checkable` para o dublê da suíte ser…, Grava a capability, casando por `name` — a chave estável em disco., CapabilityRecord, BaseModel, Uma linha da tabela `capabilities`. (+3 more)

## Knowledge Gaps
- **610 isolated node(s):** `name`, `private`, `version`, `type`, `description` (+605 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **52 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `ToolSpec` connect `ToolSpec` to `gemini_provider.py`, `SystemToolExecutor`, `ollama_provider.py`, `ProfiledToolExecutor`, `FakeLLMProvider`, `InMemoryGoalStore`, `CapabilityPermissions`, `chief.py`, `InMemoryConversationStore`, `guard_tools`, `contracts.py`, `test_agent_profiles.py`, `Event`, `RuntimeAdapter`, `llm/base.py`, `FakeEmbeddingProvider`, `Capability`, `Problema`, `ExecutionStatus`, `PythonRuntime`, `ExecutionRequest`, `GoalStore`, `LLMProvider`, `CapabilityManifest`?**
  _High betweenness centrality (0.078) - this node is a cross-community bridge._
- **Why does `CapabilityPermissions` connect `CapabilityPermissions` to `test_shell.py`, `Capability`, `Problema`, `SistemaDeArquivos`, `contracts.py`, `PermissionPolicy`, `CapabilityHarness`, `exemplo_nas/backend/handlers.py`, `InMemoryGoalStore`, `ExecutionRequest`, `Resposta`, `Http`, `test_capability_sdk_manifest.py`, `PermissaoNaoDeclarada`, `test_http.py`, `CapabilityRecord`, `CapabilityManifest`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `tool()` connect `CapabilityPermissions` to `test_shell.py`, `filesystem/backend/handlers.py`, `Capability`, `Problema`, `CapabilityHarness`, `exemplo_nas/backend/handlers.py`, `memory_writer/backend/handlers.py`, `http/schemas/models.py`, `git/backend/handlers.py`, `test_capability_sdk_manifest.py`, `PermissaoNaoDeclarada`, `._recusar`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **Are the 45 inferred relationships involving `ToolSpec` (e.g. with `ChiefAI` and `ProfiledToolExecutor`) actually correct?**
  _`ToolSpec` has 45 INFERRED edges - model-reasoned connections that need verification._
- **Are the 29 inferred relationships involving `CapabilityPermissions` (e.g. with `NasArquivos` and `Sonda`) actually correct?**
  _`CapabilityPermissions` has 29 INFERRED edges - model-reasoned connections that need verification._
- **Are the 10 inferred relationships involving `CapabilityRegistry` (e.g. with `GoalManager` and `ManifestLoadError`) actually correct?**
  _`CapabilityRegistry` has 10 INFERRED edges - model-reasoned connections that need verification._
- **Are the 28 inferred relationships involving `Event` (e.g. with `InProcEventBus` and `Subscription`) actually correct?**
  _`Event` has 28 INFERRED edges - model-reasoned connections that need verification._